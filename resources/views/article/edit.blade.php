...
@section('style')
    <link rel="stylesheet" href="/css/style/article/edit_article.css">
...
@section('container')
    ...
    <div class="row justify-content-center input-form">
        <div class="col-lg-10">
            <form action="/articles/{{ $article->slug }}" method="POST" autocomplete="off" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="mb-7">
                    <h3>
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                        value="{{ old('title', $article->title) }}" name="title" id="title"
                        aria-describedby="emailHelp" placeholder="Masukkan Judul Artikel" required autocomplete="off">
                    @error('title')
                ...
                <div class="mb-7">
                    ...
                    <select class="form-select" name="category_id" id="category_id" aria-label="Default select example">
                        @foreach ($articles_category as $article_category)
                            @if (old('category_id', $article->category_id) == $article_category->id)
                ...
                <div class="mb-7">
                    ...
                    <input type="text" name="highlight" value="{{ old('highlight', $article->highlight) }}"
                        id="highlight" class="form-control @error('highlight') is-invalid @enderror""
                        aria-describedby="emailHelp" placeholder="Masukkan teks" required autocomplete="off">
                    @error('highlight')
                        <div class="invalid-feedback fs-5">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-7">
                    ...
                    @if ($article->image)
                        <img src="{{ asset('img/' . $article->image) }}" class="img-preview img-fluid mb-3 col-sm-1"
                ...
                <div class="mb-7">
                    ...
                    <input id="deskripsi" name="body" value="{{ old('body', $article->body) }}" type="hidden" required
                        autocomplete="off">
                ...
                <div class="d-none d-xl-flex button-add justify-content-between">
                    <a class="btn btn-primary btn-back" href="/articles/{{ $article->slug }}" role="button">
